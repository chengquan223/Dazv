(function (global, factory) {
    ...
    Axis.prototype.drawTickLine = function (ctxBack) {
        ...
        for (var i = 1; i <= axisData.length; i++) {
            ...
            var text = windData[i - 1].month;
            this.drawLabel(ctxBack, text, _x, _y + opts.lableOffset, true);
        }
        for (var j = axisData[0].length; j > 0; j--) {
            ...
            var direction = windData[0].wind[axisData[0].length - j][0];
            this.drawLabel(ctxBack, direction, _x - opts.lableOffset, _y, false);
        }
        ctxBack.stroke();
        ctxBack.restore();
    };
    ...
    Axis.prototype.drawTitle = function (ctxBack) {
        ...
        ctxBack.textAlign = 'center';
        ctxBack.textBaseline = 'top';
        ctxBack.fillText('month（月份）', this.start.x + this.width / 2, this.end.y + opts.titleOffset);
        ctxBack.restore();
        ctxBack.translate(this.start.x - opts.titleOffset, this.end.y - this.height / 2);
        ctxBack.rotate(-Math.PI / 2);
        ctxBack.fillText('wind direction（风向）', 0, 0);
        ctxBack.restore();
    };
    ...
    Axis.prototype.convertGridData = function (originData, legendOpts) {
        ...
        originData.forEach(function (data, i) {
            data.wind.forEach(function (wind, j) {
                if (wind[0] == undefined || wind[0] == '') {
                    return;
                }
                ...
                var x = axisData[i][j].x,
                    y = axisData[i][j].y,
                    index = i * data.wind.length + j;
                var grid = new Grid(index, x, y, self.gridWidth, self.gridHeight);
                grid.centerx = x + self.gridWidth / 2;
                grid.centery = y + self.gridWidth / 2;
                grid.value = value;
                grid.color = color;
                grid.level = level;
                grid.date = data.month;
                grid.direction = wind[0];
                gridData.push(grid);
            });
        });
        return gridData;
